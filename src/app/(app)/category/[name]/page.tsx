...
export default function CategoryPage() {
  ...
  const categoryEventsQuery = useMemo(() => {
  ...
  }, [eventsCollectionRef, categoryName]);

  const {
    data: events,
    loading,
    error,
  } = useCollection<Event>(categoryEventsQuery);

  return (
    <div className="container mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold font-headline mb-8">Events in: <span className="text-primary">{categoryName}</span></h1>
        {loading && (
